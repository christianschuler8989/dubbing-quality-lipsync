...
def shiftLeftFrames():
	with open(pathTimestamps, "r") as timestamps:
		...
		if myShiftFactor > 0: # For shiftFactor of 0 no modifications are needed
			...
			################################################
			# Modifying the frames in front of the phoneme #
			################################################
			while frameLocator > 0: # As long as the location is in front of the phoneme:
				...
				print("Frame (Shift-LEFT) to be removed (left): " + currentFrameIdentifier)
				...
				print("Frame (Shift-LEFT) to be repositioned (left): " + currentFrameRightIdentifier)
			...
			##############################################
			# Modifying the frames of the phoneme itself #
			##############################################
			frameRelocator = 0
			...
			while numberOfFramesToBeReplacedTemp > 0: # Frames of the phoneme to be moved
				...
				currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
				print("Frame (Shift-LEFT) to be removed (middle): " + currentFrameIdentifier)
				os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
				...
				currentFrameRightIdentifier = str(currentFrameRight).zfill(5) + ".png"
				print("Frame (Shift-LEFT) to be repositioned (middle): " + currentFrameRightIdentifier)
				copyfile(pathVideoNewFrames + "/" + currentFrameRightIdentifier, pathVideoNewFrames + "/" + currentFrameIdentifier)
				#################################################################
				# Add some marker to locate the phoneme while watching the clip #
				#################################################################
			...
			while frameLocator > 1: # Number of frames right behind the phoneme that have to be copied
				# Remove Frame on the left
				currentFrame = firstFrame + frameRelocator
				...
				###########################################
				currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
				print("Frame (Shift-LEFT) to be removed (right): " + currentFrameIdentifier)
				os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
				# Replace missing frame with a frame from the right
				currentFrameRight = currentFrame + myShiftFactor - positionOfReplacementFrameCounter - 1 # The frame to be copied is as far away to the right as the number of frames of the shift #[Bug-Fix-Nov-16: -1]
				currentFrameRightIdentifier = str(currentFrameRight).zfill(5) + ".png"
				currentFrameRightPlus = currentFrame + myShiftFactor - positionOfReplacementFrameCounter # The frame to be copied is as far away to the right as the number of frames of the shift #[Bug-Fix-Nov-16: -1 (removed the +1)]
				currentFrameRightIdentifierPlus = str(currentFrameRightPlus).zfill(5) + ".png"
				print("Frame (Shift-LEFT) to be repositioned (right): " + currentFrameRightIdentifier + " and " + currentFrameRightIdentifierPlus)
				...
				if frameLocator > 1: # Prevent the removal of the last frame and then trying to replace it with itself
					...
					#currentFrameRightIdentifier = str(currentFrameRight).zfill(5) + ".png"
					print("Frame (Shift-LEFT) to be repositioned (right): " + currentFrameRightIdentifierPlus)
					copyfile(pathVideoNewFrames + "/" + currentFrameRightIdentifierPlus, pathVideoNewFrames + "/" + currentFrameIdentifier)
					#################################################################
					# Add some marker to locate the phoneme while watching the clip #
					#################################################################
					#myCommandToAddMarker = "composite -geometry +20+20 " + pathVideoFrames + "/../../../../../images/markTimoG.jpg" + " " + pathVideoNewFrames + "/" + currentFrameIdentifier + " " + pathVideoNewFrames + "/" + currentFrameIdentifier
...
###########################################################################################################
# Copy frames for the phonemes inside the timewindow (all phonemes from timestamps.txt)                   #
# But delete every second frame behind the phoneme in a range of shift-factor and compensate in the start #
###########################################################################################################
def shiftRightFrames():
	with open(pathTimestamps, "r") as timestamps:
		...
		if myShiftFactor > 0: # For shiftFactor of 0 no modifications are needed
	        ...
	        #indexOfMiddlePhoneme = int(numberOfPhonemesNeedingFrames / 2) 	# For accessing the middle of the phoneme
			# This should be correct in 50% of the cases in which the phoneme has only one single frame of duration
			# or is too close to the boarder of the original video to actually fill the allocated timewindow around the phoneme (solved by introducing buffer-windows beforehand)
			...
			if numberOfFramesToBeReplaced < 1:
				print("The number of frames to be replaced is smaller than 1 and therefor nothing can be done.")
			currentFrame = indexOfEndFrame #[Change for new Shift to the right]

			frameLocator = myShiftFactor 	# How far we went
			frameRelocator = 0 				# How far we have to go
			lastFrame = currentFrame + (frameLocator * 2) #[Change for new Shift to the right]
			# [TODO(?): Check if frame is outside video. But the buffers added beforehand should be enough already]
			#if (currentFrame + (frameLocator * 2) < 1):
				# If the window for modifications is outside the video- start with last frame instead
			#	lastFrame = identifierOfLastFrame???
			#else:
			#	lastFrame = currentFrame - (frameLocator * 2)

			##############################################
			# Modifying the frames behind of the phoneme #
			##############################################
			while frameLocator > 0: # As long as the location is in front of the phoneme:
				# Remove Frame on the left
				currentFrame = lastFrame + frameRelocator
				frameRelocator -= 1		# The next frame should be taken from further on the left #[Change for new Shift to the right]
				currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be removed (right): " + currentFrameIdentifier)
				os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
				# Replace missing frame with correspoding left neighbour (which might not be a direct neighbour)
				currentFrameLeft = currentFrame + frameRelocator
				currentFrameLeftIdentifier = str(currentFrameLeft).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be repositioned (right): " + currentFrameLeftIdentifier)
				copyfile(pathVideoNewFrames + "/" + currentFrameLeftIdentifier, pathVideoNewFrames + "/" + currentFrameIdentifier)
				#################################################################
				# Add some marker to locate the phoneme while watching the clip #
				#################################################################
				#myCommandToAddMarker = "composite -geometry +20+20 " + pathVideoFrames + "/../../../../../images/markTimoB.jpg" + " " + pathVideoNewFrames + "/" + currentFrameIdentifier + " " + pathVideoNewFrames + "/" + currentFrameIdentifier
			...
			while numberOfFramesToBeReplacedTemp > 0: # Frames of the phoneme to be moved
				...
				# [TODO(?): Check if frame is outside video. But the buffers added beforehand should be enough already]
				#if (indexOfEndFrame + myShiftFactor < 1):
				#	currentFrame = 1
				#else:
				#	currentFrame = indexOfEndFrame + myShiftFactor # How far to the right to move
				currentFrame = indexOfEndFrame + myShiftFactor # How far to the right to move #[Change for new Shift to the right]
				currentFrame -= frameRelocator # Displacement for moving to the left with the index #[Change for new Shift to the right]
				currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be removed (middle): " + currentFrameIdentifier)
				os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
				# Replace missing frame with left direct neighbour
				currentFrameLeft = indexOfEndFrame - frameRelocator #[Change for new Shift to the right]
				#print("indexOfStartFrame: " + str(indexOfStartFrame))
				#print("frameRelocator: " + str(frameRelocator))
				currentFrameLeftIdentifier = str(currentFrameLeft).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be repositioned (middle): " + currentFrameLeftIdentifier)
				copyfile(pathVideoNewFrames + "/" + currentFrameLeftIdentifier, pathVideoNewFrames + "/" + currentFrameIdentifier)
				#################################################################
				# Add some marker to locate the phoneme while watching the clip #
				#################################################################
				...
				frameRelocator += 1		# Move index of frame for replacement by one
				numberOfFramesToBeReplacedTemp -= 1 # Keep going until the beginning of the phoneme has been reached

			#############################################
			# Modify the frames right before he phoneme #
			#############################################
			frameLocator = myShiftFactor 		# Where to remove frames from- wrt. endFrame
			frameRelocator = 0					# How far to move frames to replace just removed frames
			lastFrame = indexOfEndFrame + (myShiftFactor) - numberOfFramesToBeReplaced # Starting position of operation to the left #[Change for new Shift to the right]
			# Beginning at the indexOfEndFrame, adding the number of frames that have been removed, subtracting the size of the phoneme to end up right in front of the last modified frame

			# Modify the frames in front of the phoneme
			positionOfReplacementFrameCounter = 0
			while frameLocator > 1: # Number of frames right in front of the phoneme that have to be copied
				# Remove Frame on the right
				currentFrame = lastFrame - frameRelocator #[Change for new Shift to the right]
				frameRelocator += 2 # Move index to the left by 2 more than before
				# This way the index (frameRelocator) will skip the second to be replaced frame
				###########################################
				# First frame of the loop to be replaced #
				###########################################
				currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be removed (left): " + currentFrameIdentifier)
				os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
				# Replace missing frame with a frame from the left
				currentFrameLeft = currentFrame - myShiftFactor + positionOfReplacementFrameCounter # The frame to be copied is as far away to the left as the number of frames of the shift #[Change for new Shift to the right]
				currentFrameLeftIdentifier = str(currentFrameLeft).zfill(5) + ".png"
				currentFrameLeftPlus = currentFrame - myShiftFactor + positionOfReplacementFrameCounter + 1 # The frame to be copied is as far away to the left as the number of frames of the shift #[Change for new Shift to the right]
				currentFrameLeftIdentifierPlus = str(currentFrameLeftPlus).zfill(5) + ".png"
				print("Frame (Shift-RIGHT) to be repositioned (left): " + currentFrameLeftIdentifier + " and " + currentFrameLeftIdentifierPlus)
				# Instead of plainly copying the frame twice, the first "copy" is a combination of two neighbouring frames to make the transition more smooth
				myCommandToBlendImages = "composite -blend 50 -gravity South " + pathVideoNewFrames + "/" + currentFrameLeftIdentifier + " " + pathVideoNewFrames + "/" + currentFrameLeftIdentifierPlus + " -alpha Set " + pathVideoNewFrames + "/" + currentFrameIdentifier
				os.system(myCommandToBlendImages)
				# Base-command: composite -blend 50 -gravity South 00016.png 00017.png -alpha Set 0001617.png
				# Plain-Copy-Version without blending: copyfile(pathVideoNewFrames + "/" + currentFrameRightIdentifier, pathVideoNewFrames + "/" + currentFrameIdentifier)
				#################################################################
				...
				###########################################
				# Same procedure for the direct neighbour of the just processed frame
				# (this way the number of frames, removed from right after the shifted phoneme
				# and the number of frames added right in front of the phoneme will be ballancing each other out)
				if frameLocator > 1: # Prevent the removal of the last frame and then trying to replace it with itself
					currentFrame -= 1 #[Change for new Shift to the right]
					currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
					print("Frame (Shift-RIGHT) to be removed (left): " + currentFrameIdentifier)
					os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
					# Again replace the missing frame with the same frame from the left as before (copying this frame)
					#currentFrameRight = currentFrame + numberOfFramesToBeReplaced # The frame to be copied is as far away to the right as the number of frames of the shift
					#currentFrameRightIdentifier = str(currentFrameRight).zfill(5) + ".png"
					print("Frame (Shift-RIGHT) to be repositioned (left): " + currentFrameLeftIdentifier)
					copyfile(pathVideoNewFrames + "/" + currentFrameLeftIdentifier, pathVideoNewFrames + "/" + currentFrameIdentifier)
					#################################################################
					# Add some marker to locate the phoneme while watching the clip #
					#################################################################
...
############################################################################################################
# Copy frames for the phonemes inside the timewindow (all phonemes from timestamps.txt)                    #
# But delete the frames of the phoneme and replace them with a "melded" combination of neighbouring frames #
############################################################################################################
def meldFrames():
	with open(pathTimestamps, "r") as timestamps:
		phonemesNeedingFrames = timestamps.readlines()
		...
		# Copy all the frames for video-generation
		print("Copy all the frames for video-generation") # Debugging
		for frame in os.listdir(pathVideoFrames):
			frameIdentifier = frame.split(".")[0]
			if int(startFrameOfTimewindow) <= int(frameIdentifier) & int(frameIdentifier) <= int(endFrameOfTimewindow):
				copyfile(pathVideoFrames+frame, pathVideoNewFrames + "/" + frame)
		...
		indexOfNeighbourLeft = indexOfStartFrame - 1
		indexOfNeighbourRight = indexOfEndFrame + 1
		...
		while numberOfFramesToBeReplacedTemp > 0: # Frames of the phoneme to be moved
			...
			###########################################
			currentFrameIdentifier = str(currentFrame).zfill(5) + ".png"
			print("Frame (Shift-MELD) to be removed (right): " + currentFrameIdentifier)
			os.remove(pathVideoNewFrames + "/" + currentFrameIdentifier)
			# Replace missing frame with a frame from the right
			...
			currentFrameNeighbourLeft = str(indexOfNeighbourLeft).zfill(5) + ".png"
			currentFrameNeighbourRight = str(indexOfNeighbourRight).zfill(5) + ".png"
			...
			#currentFrameRightIdentifierPlus = str(currentFrameRightPlus).zfill(5) + ".png"
			print("Frame (Shift-MELD) to be repositioned (right): " + currentFrameNeighbourLeft + " and " + currentFrameNeighbourRight)
			# Instead of plainly copying the frame twice, the first "copy" is a combination of two neighbouring frames to make the transition more smooth
			myCommandToBlendImages = "composite -blend 50 -gravity South " + pathVideoNewFrames + "/" + currentFrameNeighbourLeft + " " + pathVideoNewFrames + "/" + currentFrameNeighbourRight + " -alpha Set " + pathVideoNewFrames + "/" + currentFrameIdentifier
			os.system(myCommandToBlendImages)
			...
			#frameRelocator += 1
			numberOfFramesToBeReplacedTemp -= 1 # Keep going until the end of the phoneme has been reached
			currentFrame += 1 # Move index of frame for replacement by one
