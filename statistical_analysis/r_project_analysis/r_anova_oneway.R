...
do.call(sample_n_by, list(data = data_anch, eval(quote(identifier)), size = 1), quote = FALSE)


data_anch$eval(quote(identifier)) <- as.factor(data_anch$eval(quote(identifier))) # "Char" have no levels
sink(sink_path)
## Levels ######################################################################
levels(data_anch$eval(quote(identifier)))
sink()
...
model <- lm(Grade ~ Excerpt, data = data_anch)
...
list_of_plots[[3]] <- ggqqplot(data_anch, "Grade", facet.by = "Excerpt", main = "Check normality assumption by groups")
...
sink(sink_path, append = TRUE)
## Computation #################################################################
res.aov <- data_anch %>% anova_test(Grade ~ Excerpt)
res.aov
sink()
...
pwc <- data_anch %>% tukey_hsd(Grade ~ Excerpt)
pwc
sink()
...
list_of_plots[[5]] <- ggboxplot(data_anch, x = "Excerpt", y = "Grade", main = "ANOVA box plots with p-values") +
