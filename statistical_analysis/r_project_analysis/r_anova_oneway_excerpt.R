...
data_anch %>% sample_n_by(Excerpt, size = 1)
...
#### Visualization
list_of_plots[[1]] <- ggboxplot(data_anch, x = "Excerpt", y = "Grade", main = "Summary statistics")
list_of_plots[[2]] <- ggboxplot(data_anch, x = "Excerpt", y = "Rank", main = "Summary statistics")
...
model_grade <- lm(Grade ~ Excerpt, data = data_anch)
model_rank <- lm(Rank ~ Excerpt, data = data_anch)
...
list_of_plots[[5]] <- ggqqplot(data_anch, "Grade", facet.by = "Excerpt", main = "Check normality assumption by groups")
list_of_plots[[6]] <- ggqqplot(data_anch, "Rank", facet.by = "Excerpt", main = "Check normality assumption by groups")
...
data_anch %>% levene_test(Grade ~ Excerpt)
data_anch %>% levene_test(Rank ~ Excerpt)
sink()

#### Computation ###############################################################
sink(sink_path, append = TRUE)
## Computation #################################################################
res_grade.aov <- data_anch %>% anova_test(Grade ~ Excerpt)
res_grade.aov
res_rank.aov <- data_anch %>% anova_test(Rank ~ Excerpt)
res_rank.aov
sink()
...
pwc_grade <- data_anch %>% tukey_hsd(Grade ~ Excerpt)
pwc_grade
pwc_rank <- data_anch %>% tukey_hsd(Rank ~ Excerpt)
pwc_rank
sink()
...
list_of_plots[[9]] <- ggboxplot(data_anch, x = "Excerpt", y = "Grade", main = "ANOVA box plots with p-values") +
...
list_of_plots[[10]] <- ggboxplot(data_anch, x = "Excerpt", y = "Rank", main = "ANOVA box plots with p-values") +
