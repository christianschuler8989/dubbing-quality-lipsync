...
data_anch <- data_done %>% 
  ...
  select(ResultID, Excerpt, Attribute, Modification, Grade,
         Runtime, SubjectName, Object, TrialID, Rank, SubjectInterest) # Tighten the scope
#SubjectAge, SubjectSex, SubjectInterest, SubjectLanguage, SessionID, SeqNum,

data_anch %>% sample_n_by(SubjectInterest, size = 1)

data_anch$SubjectInterest <- as.factor(data_anch$SubjectInterest) # "Char" have no levels
sink(sink_path)
## Levels ######################################################################
levels(data_anch$SubjectInterest)
sink()
...
#### Visualization
list_of_plots[[1]] <- ggboxplot(data_anch, x = "SubjectInterest", y = "Grade", main = "Summary statistics")
list_of_plots[[2]] <- ggboxplot(data_anch, x = "SubjectInterest", y = "Rank", main = "Summary statistics")
...
model_grade <- lm(Grade ~ SubjectInterest, data = data_anch)
model_rank <- lm(Rank ~ SubjectInterest, data = data_anch)
...
list_of_plots[[5]] <- ggqqplot(data_anch, "Grade", facet.by = "SubjectInterest", main = "Check normality assumption by groups")
list_of_plots[[6]] <- ggqqplot(data_anch, "Rank", facet.by = "SubjectInterest", main = "Check normality assumption by groups")
...
data_anch %>% levene_test(Grade ~ SubjectInterest)
data_anch %>% levene_test(Rank ~ SubjectInterest)
sink()

#### Computation ###############################################################
sink(sink_path, append = TRUE)
## Computation #################################################################
res_grade.aov <- data_anch %>% anova_test(Grade ~ SubjectInterest)
res_grade.aov
res_rank.aov <- data_anch %>% anova_test(Rank ~ SubjectInterest)
res_rank.aov
sink()
...
pwc_grade <- data_anch %>% tukey_hsd(Grade ~ SubjectInterest)
pwc_grade
pwc_rank <- data_anch %>% tukey_hsd(Rank ~ SubjectInterest)
pwc_rank
sink()
...
pwc_grade <- pwc_grade %>% add_xy_position(x = "SubjectInterest")
list_of_plots[[9]] <- ggboxplot(data_anch, x = "SubjectInterest", y = "Grade", main = "ANOVA box plots with p-values") +
...
pwc_rank <- pwc_rank %>% add_xy_position(x = "SubjectInterest")
list_of_plots[[10]] <- ggboxplot(data_anch, x = "SubjectInterest", y = "Rank", main = "ANOVA box plots with p-values") +
