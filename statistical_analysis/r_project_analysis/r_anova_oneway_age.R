...
data_anch <- data_done %>% 
  ...
  select(ResultID, Excerpt, Attribute, Modification, Grade,
         Runtime, SubjectName, Object, TrialID, Rank, SubjectAge) # Tighten the scope
#SubjectAge, SubjectSex, SubjectInterest, SubjectLanguage, SessionID, SeqNum,

data_anch %>% sample_n_by(SubjectAge, size = 1)

data_anch$SubjectAge <- as.factor(data_anch$SubjectAge) # "Char" have no levels
sink(sink_path)
## Levels ######################################################################
levels(data_anch$SubjectAge)
sink()
...
#### Visualization
list_of_plots[[1]] <- ggboxplot(data_anch, x = "SubjectAge", y = "Grade", main = "Summary statistics")
list_of_plots[[2]] <- ggboxplot(data_anch, x = "SubjectAge", y = "Rank", main = "Summary statistics")
...
model_grade <- lm(Grade ~ SubjectAge, data = data_anch)
model_rank <- lm(Rank ~ SubjectAge, data = data_anch)
...
list_of_plots[[5]] <- ggqqplot(data_anch, "Grade", facet.by = "SubjectAge", main = "Check normality assumption by groups")
list_of_plots[[6]] <- ggqqplot(data_anch, "Rank", facet.by = "SubjectAge", main = "Check normality assumption by groups")
...
data_anch %>% levene_test(Grade ~ SubjectAge)
data_anch %>% levene_test(Rank ~ SubjectAge)
sink()

#### Computation ###############################################################
sink(sink_path, append = TRUE)
## Computation #################################################################
res_grade.aov <- data_anch %>% anova_test(Grade ~ SubjectAge)
res_grade.aov
res_rank.aov <- data_anch %>% anova_test(Rank ~ SubjectAge)
res_rank.aov
sink()
...
pwc_grade <- data_anch %>% tukey_hsd(Grade ~ SubjectAge)
pwc_grade
pwc_rank <- data_anch %>% tukey_hsd(Rank ~ SubjectAge)
pwc_rank
sink()
...
pwc_grade <- pwc_grade %>% add_xy_position(x = "SubjectAge")
list_of_plots[[9]] <- ggboxplot(data_anch, x = "SubjectAge", y = "Grade", main = "ANOVA box plots with p-values") +
...
pwc_rank <- pwc_rank %>% add_xy_position(x = "SubjectAge")
list_of_plots[[10]] <- ggboxplot(data_anch, x = "SubjectAge", y = "Rank", main = "ANOVA box plots with p-values") +
