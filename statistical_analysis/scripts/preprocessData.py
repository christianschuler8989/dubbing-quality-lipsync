...
pathInputTextfile = "./../data/" + study_run + "/data_raw/"

from os import listdir
from os.path import isfile, join

# Only take names of files not of directories
inputTextfiles = [pathInputTextfile+f for f in listdir(pathInputTextfile) if isfile(join(pathInputTextfile, f))]

# Define path to output-csv-files
pathOutputCsv = "./../data/" + study_run + "/data_preprocessed/"
outputCsv = pathOutputCsv + "dataPreprocessed.csv"
...
#################################################################
# A: Somehow distinguish the participants (creators of results) #
#################################################################

######################################################################################
# B: Read or sort results by ascending date (these should also be the largest files) #
######################################################################################

############################################################
# A & B: Consider only the last result of each participant #
############################################################

###########################################################
# C: Read the result of one participant for preprocessing #
###########################################################
import json
...
def resultTxtReader(inputTextfile):
    ...
    #print("Data type after reconstruction: ", type(dataJsonDictionary))
    #print(dataJsonDictionary)
...
for inputTextfile in inputTextfiles:
    ...
    userMetasAndUserDatas.append([userMeta, userData])
# => userDatasAndUserMetas = [ [userData1, userMeta1], [userData2, userMeta2], ...]
#print("userMetasAndUserDatas")
#print(userMetasAndUserDatas)

#userMeta01, userData01 = resultTxtReader(inputTXT01)
#userMeta02, userData02 = resultTxtReader(inputTXT02)
...
def txtToDictTransformer(userMeta, userData):
    ...
    for trialResult in userData:
        ...
        for key, value in view_on_grading:
            ...
            # Special cases for the mixed-Trials (named "all")
            #if boolean_mixedTrialOfAll == True:
            #    current_item = current_filenames[key].split("_")[3]
            ...
            if current_excerpt == "Anfang":
                ...
                if current_item == "a":
                    ...
                    # This is the case for the mixed-trials (named "all" and containing more than one item)
                    # boolean_mixedTrialOfAll = True
            ...
            userDict = dict([
                ...
                ("ObjRight2", objRight2),
                ("ObjRight3", objRight3),
                ("ObjLeft3Length", objLeft3L),
                ("ObjLeft2Length", objLeft2L),
                ("ObjLeft1Length", objLeft1L),
                ("ObjectLength", objectL),
                ("ObjRight2Length", objRight1L),
                ("ObjRight1Length", objRight2L),
                ("ObjRight3Length", objRight3L),
                ("ObjLeft3Type", objLeft3T),
                ("ObjLeft2Type", objLeft2T),
                ("ObjLeft1Type", objLeft1T),
                ("ObjectTye", objectT),
                ("ObjRight1Type", objRight1T),
                ("ObjRight2Type", objRight2T),
                ("ObjRight3Type", objRight3T)
            ])
            #print("")
            #print(userDict)
...
#userDictArray01 = txtToDictTransformer(userMeta01, userData01)

#userDictArray02 = txtToDictTransformer(userMeta02, userData02)

userDictArrays = []

print("Transform trial-results into arrays of dictionaries.")
for pairOfUserData in userMetasAndUserDatas:
    currentUserDictArray = txtToDictTransformer(pairOfUserData[0], pairOfUserData[1])
    userDictArrays.append(currentUserDictArray)
# => userDictArrays = [[[user01Dict01], [user01Dict02], ...], [[user02Dict01], ...], ...]
#print("userDictArrays")
#print(userDictArrays[0])
#print("userDictArrays")
#print(userDictArrays[1])
#print("userDictArrays")
#print(userDictArrays[2])

#print(userDictArray01)
#print(userDictArray02)
...
#   #########################################
#   # For each finished test in this result #
#   #########################################
...
#   ######################################
#   # Combine with participants-metadata #
#   ######################################

#   #################################################
#   # Into new dictionary => new line in output.csv #
#   #################################################
...
###################################################################################
# Filling of the remaining slots for "Objects", "ObjectLengths" and "ObjectTypes" #
###################################################################################
# => According to the corresponding excert (Anfang, Europaeisch, Paar, Pandemie, Tempo)


##############################################################
# Filling empty spaces with "NA" to get an R-friendly format #
##############################################################
# => Is this easier here in python or inside the (almost) finished .csv??
    ...
    with open(outputCsv, 'w', newline='') as csvfile:
        ...
        for userDictArray in userDictArrays:
            for row in userDictArray:
                writer.writerow(row)
        #for row in userDictArray01:
        #    writer.writerow(row)
        #for row in userDictArray02:
        #    writer.writerow(row)
        #writer.writerow({'Excerpt': 'Baked', 'Item': 'Beans'})
        #writer.writerow({'FileLow': 'Lovely', 'File0': 'Spam'})
        #writer.writerow({'ObjLeft2Type': 'Wonderful', 'ObjectTye': 'Spam'})

#for userDictionaryArray in userDictArrays:
