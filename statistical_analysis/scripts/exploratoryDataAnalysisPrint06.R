...
plot_global_pdf <- function(pdf_boolean,
  ...
  list_of_plot_titles[[1]] <- "Ranks per Attribute for  (Anfang-a)"
  list_of_dataframes[[2]] <- filter(data, Excerpt == "Anfang", Object == "p")
  list_of_plot_titles[[2]] <- "Ranks per Attribute for  (Anfang-p)"
  ...
  list_of_plot_titles[[3]] <- "Ranks per Attribute for  (Europaeisch-a)"
  list_of_dataframes[[4]] <- filter(data, Excerpt == "Europaeisch", Object == "p")
  list_of_plot_titles[[4]] <- "Ranks per Attribute for  (Europaeisch-p)"
  ...
  list_of_plot_titles[[5]] <- "Ranks per Attribute for  (Impfangebot-w)"
  ...
  list_of_plot_titles[[6]] <- "Ranks per Attribute for  (Paar-a)"
  list_of_dataframes[[7]] <- filter(data, Excerpt == "Paar", Object == "p")
  list_of_plot_titles[[7]] <- "Ranks per Attribute for  (Paar-p)"
  list_of_dataframes[[8]] <- filter(data, Excerpt == "Paar", Object == "w")
  list_of_plot_titles[[8]] <- "Ranks per Attribute for  (Paar-w)"
  ...
  list_of_plot_titles[[9]] <- "Ranks per Attribute for  (Pandemie-a)"
  list_of_dataframes[[10]] <- filter(data, Excerpt == "Pandemie", Object == "p")
  list_of_plot_titles[[10]] <- "Ranks per Attribute for  (Pandemie-p)"
  ...
  list_of_plot_titles[[11]] <- "Ranks per Attribute for  (Tempo-a)"
  list_of_dataframes[[12]] <- filter(data, Excerpt == "Tempo", Object == "p")
  list_of_plot_titles[[12]] <- "Ranks per Attribute for  (Tempo-p)"
  list_of_dataframes[[13]] <- filter(data, Excerpt == "Tempo", Object == "w")
  list_of_plot_titles[[13]] <- "Ranks per Attribute for  (Tempo-w)"
  ...
  list_of_plot_titles[[14]] <- "Ranks per Attribute for  (Zusammen-a)"
  ...
  if (pdf_boolean == TRUE) {
    ...
    pdf(file = paste0("expl_06_global_rank_", identifier, ".pdf"))
  ...
  if (png_boolean == TRUE) {
    ...
    for (plot in list_of_plots) {
      ...
      png(
        file = paste0("plots/", "expl-06-global-rank", image_counter_padded, ".png"),
        width = image_width,
        height = image_height
      )
      print(plot)
